...
- الحالة: مدموج ومنشور — آخر دمج `3618746` (2026-07-26) يضم **شاشة «الفواتير
  السابقة»** داخل فاتورة المبيعات. النسخة المنشورة **tobacco-116 / v388**
  (الأعلى على main كان v387 برفع تلقائي من مولّد النشرات، فرُفع إلى v388).
- الدمج السابق `de6ee23` (tobacco-115 / v386): إصلاح تشويه أصناف السطر المدمج،
  ورسالة شفافية الحفظ، وتنبيه «تحت التكلفة» المعلوماتي، واعتبار سعر الجملة
  الصفري «غير مسعّر».
- مهمة الترقيم `TOBACCO Invoice Series Push` **مسجّلة وتعمل** كل 5 دقائق، وآخر
  رفع ناجح موثّق في `tools/logs/invoice-series-push.log`.
- **معلّق على المالك:** تجربة الهاتف — الطباعة، اقتراح الزبون، رقم الفاتورة،
  كود الأمين `0000`، ثبات عمود اسم الصنف، آخر سعر للزبون، وتحذير ⛔ لأحد
  المتجاوزين الثمانية. **الرقم المتوقَّع ليس ثابتاً**: هو دائماً «التالي» في
  سلسلة الأمين وقت الفتح، ويتقدّم مع كل فاتورة حقيقية. عند 2026-07-26 08:03 UTC
  كان **209 للجملة و616 للمفرق** (وكان 202/601 يوم كتابة الطلب الأصلي).
- حدّ معروف في الترقيم: الموقع لا يكتب في الأمين، فلا يحجز رقماً فيه. نافذة
  التكرار قُلّصت إلى ≤5 دقائق (دورة المهمة) ولم تُغلق تماماً. الإغلاق الكامل
  يحتاج حقل رقم قابل للتعديل — معروض على المالك ولم يُطلب بعد.
- المهمة المفتوحة: وحدة الفاتورة — نُشر حتى الآن المتطلبات 9 و16 و22 و23؛ والباقي من 1–23 قادم
- **ملف الأسعار — قرارات تسعير معلّقة على المالك** (تحليل حيّ 2026-07-26 مقابل
  `item_costs`): أربعة أصناف فقط تحت التكلفة ولها مخزون — نخلة صلاحية شهر 10
  (325$ مقابل تكلفة 434.55$، سبع شرحات، تعرّض −767$)، فحم الزعيم 250غ (−182$)،
  معسل نخلة اخضر كروز (−40$)، غلواز قصير أصفر (−0.6$). وخمسة أصناف تحت التكلفة
  بلا مخزون (أخطرها مانشستر كوين طقة توت 210$ مقابل تكلفة 336$). و54 صنفاً
  سعرها أقدم من 30 يوماً مع مخزون، منها 16 هامشها تحت 5٪. **البيع تحت التكلفة
  مسموح ومقصود أحياناً** — التنبيه معلوماتي فقط ولا يمنع حفظاً ولا مزامنة.
- المسؤول: —
