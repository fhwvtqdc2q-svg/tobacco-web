...
- Branch: feature/sales-purchase-returns (PR #37، ما زال Draft، لم يُدمَج، لم أعتمده بنفسي)
- Files: src/app.js, src/supabase-client.js, scripts/check.mjs, supabase/returns-table.sql (مقترح غير مُطبَّق), tools/ameen-returns-config.json, tools/discover-ameen-returns-schema.ps1
- Result (حسب الترقيم العشرة الذي حدده المالك):
  1. سعر مرتجع المشتريات أصبح من `item.price` (سعر السطر الخام بفاتورة الشراء الأصلية) حصراً؛ `lastPrice`/`avgPrice` أصبحا حقل `unitCost` منفصلاً (أساس تكلفة فقط، لا يُستخدَم للتسعير) — `retSelectOrigInvoice()` في src/app.js.
  2. الوحدة الأصلية تُحفَظ كما وردت بالسطر فعلياً (`item.unit`/`line.unit1`/`line.unit2` حسب `invoicePriceBasis()`) بلا أي افتراض/تحويل قسري لـ unit2؛ `normalizeReturnItems()` في supabase-client.js أصبح يقبل `unit` كنص حر بدل enum.
  3. هوية كل سطر مرتجع أصبحت `retLineKey(invoiceGuid, matKey, index)` — GUID الفاتورة الأصلية + مفتاح المادة + رقم السطر — مخزَّنة كـ`line_key`، بدل رقم الفاتورة وحده.
  4. مقترح `supabase/returns-table.sql` (لم يُطبَّق على الإنتاج): حارس ذري `returns_guard_status_transition()` يستخدم `pg_advisory_xact_lock(hashtext(new.original_invoice_guid))` ضمن نفس المعاملة لمنع تجاوز السقف عند تزامن معتمِدين — تعليق بالكود يوثّق حجة الذرّية صراحة.
  5. حقول المحتوى المالي (kind, counterparty, original invoice ref, payment method, register/cash source, item lines, quantities, prices, cost, total) مقفلة بعد approved عبر `returns_guard_immutable_and_stamp()` بالمقترح SQL + دفاع بعمق بالعميل: `setReturnDocumentStatus` يرفض صراحة `nextStatus === "approved"` (يوجّه لمسار approveReturnDocument فقط)، وأُضيفت `correctReturnDocument(id, note, patch)` تطابق نمط `correctPurchaseInvoice` (correction_log/correction_count) — **لم تُربَط بعد بأي زر بالواجهة**، متاحة كـ API فقط حتى الآن؛ القرار موثّق هنا صراحة وليس كتماناً.
  6. السبب إلزامي قبل الاعتماد بالواجهة (`retSetStatus` في app.js) وبـsupabase-client.js (`approveReturnDocument` يرمي خطأً عربياً إن كان `doc.reason` فارغاً) وبقاعدة البيانات المقترحة (`constraint returns_reason_required_after_draft`).
  7. `tools/discover-ameen-returns-schema.ps1` لم يعد يقرأ `tools\.env` إطلاقاً؛ يقبل فقط `AMEEN_SQL_CONNECTION_STRING` من بيئة العملية المُصدَّرة مسبقاً، ويرفض العمل صراحةً (`throw`) إن كان `AMEEN_SQL_WRITE_CONNECTION_STRING` موجوداً في البيئة أصلاً. لم يُشغَّل — قراءة/تعديل فقط.
  8. GUID السلاسل الثلاث في `tools/ameen-returns-config.json` مُدخَلة كما أعطاها المالك حرفياً (بأحرف صغيرة lowercase، مطابقةً لصيغة الملف الحالية — `src/app.js` لا يحوي `SALES_AMEEN_SERIES` بصيغة مختلفة تستوجب تحويلاً): `2f20674c-2d81-45ff-a513-a0a160c3bfee` (مرتجع مبيعات جملة)، `ba87ac60-a404-4c68-80c4-7db1ddf6b5cf` (مرتجع مبيعات مركز)، `c9aca8fe-f50e-46eb-91ac-29ee32acbb3e` (مرتجع مشتريات) — **غير مؤكَّدة بعد من bt000 الفعلية**، موثّق صراحة بالملف نفسه أنها تصريح مباشر من المالك بانتظار اكتشاف حقيقي قبل أي Apply.
  9. **مغلق جزئياً بصدق:** `approveReturnDocument()` الجديدة بـsupabase-client.js تستدعي فعلاً `retCalc.retInvoiceProfitReversal` و`retCalc.retSettlementImpact` وتُثبِّت نتائجهما كحقول حقيقية على سجل المرتجع نفسه (`reversed_revenue/cost/profit`, `settlement_type/target_id/amount`) — وليست محسوبة ومهملة. أثر المخزون مطبَّق فعلياً ومكتوباً على `approved_price_items.stock_qty` (وليس محسوباً فقط) عبر `applyReturnStockAdjustment()`، بأساس best-effort لكل صنف: فشل مطابقة الوحدة/الصنف بثقة يُجمَّع بمصفوفة `stockWarnings` وتُعرَض صراحة بالواجهة (`retSetStatus` في app.js) بدل الفشل الصامت أو إيقاف الاعتماد المالي بالكامل.
     - **ما تبقى صادقاً وغير مكتمل:** لا يوجد دفتر أرصدة زبائن/موردين قابل للكتابة بهذا المستودع بعد — لذا "أثر التسوية" محفوظ فقط كحقل بيانات على `returns` نفسه (settlement_type/target_id/amount)، وليس مطبَّقاً فعلياً على أي رصيد خارجي حقيقي (لا يوجد جدول أرصدة عملاء/موردين مركزي حالياً). `settlement_target_id` يخزّن أياً من supplierId/customerId/treasuryId كنص واحد، يُميَّز فقط عبر `settlement_type` — تبسيط مقصود موثّق هنا. `treasuryId` هو فعلياً `doc.treasuryName` النصي الحر (لا يوجد مفهوم GUID/رقم صندوق منفصل بالكود الحالي). حقل `unitCost` الحقيقي لكل سطر مشتريات غير متوفر من `pull-purchase-invoices-from-ameen.ps1` (لا يُصدِّر تكلفة خام لكل سطر) — لذا `lastPrice`/`avgPrice` يبقيان أقرب تقريب لأساس التكلفة فقط، وليسا سعر التسعير (ذلك مضمون ببند 1). المستند يبقى بحالته الحقيقية (approved، وليس sync_pending وهمياً) لأن كل الآثار المحسوبة أعلاه فعلاً مُثبَّتة ومحفوظة، لا مجرد مُخطَّطة.
  10. أُضيفت اختبارات انحدار حقيقية بـ`scripts/check.mjs`: (أ)-(د) و(6) كفحوص شكل كود حقيقية (contract checks) على src/app.js وsupabase/returns-table.sql لأن الدوال المعنية (retSelectOrigInvoice/retPriorReturnsForItem) مرتبطة بـstate/DOM ولا يمكن عزلها بـvm؛ (و) اختبار حقيقي كامل يحمّل returns-calc.js + supabase-client.js معاً بـvm sandbox (appConfig فارغ يُجبر مسار localStorage) ويستدعي `dataStore.approveReturnDocument()` الفعلي — يتحقق من: رفض الاعتماد بلا سبب، وقيم reversed_revenue/cost/profit وsettlement_type/amount وstock_applied المحفوظة فعلياً بعد الاعتماد، وأن `approved_price_items.stock_qty` تحدّث فعلياً (148 = 100 + 4 كرتونة × عامل تحويل 12) بالوحدة الأصلية دون تحويل قسري.
- Verification: `npm.cmd run check` نجح بالكامل (كل الاختبارات الجديدة ضمنها)؛ `git diff --check` لا يُظهر شيئاً (نظيف)؛ تحقّق تشفير UTF-8 BOM لـ`tools/discover-ameen-returns-schema.ps1` (efbbbf) والتحليل النحوي عبر `[System.Management.Automation.Language.Parser]::ParseFile` نجح بلا أخطاء.
- Pending: (1) لا اكتشاف فعلي بعد لمطابقة GUID السلاسل الثلاث مع bt000 الحقيقية — لا Apply قبل ذلك. (2) `supabase/returns-table.sql` يبقى مقترحاً غير مُطبَّق على الإنتاج. (3) لا واجهة مستخدم بعد لاستدعاء `correctReturnDocument` (متاحة كـAPI فقط). (4) لا دفتر أرصدة زبائن/موردين قابل للكتابة — أثر التسوية محفوظ على سجل المرتجع فقط. (5) `sync-returns-to-ameen.ps1` يبقى مقفلاً بـ`exit 1` غير مشروط كما هو، ولم يُشغَّل أي سكريبت من tools/ ولم يُقرأ tools\.env بهذا السجل.
